...
const createCard = (req, res) => {
  ...
  form.parse(req, async (error, fields, files) => {
    ...
    jo.rotate(buffer, options, async function(
      error,
      buffer,
      orientation,
      dimensions
...
/////////////////////////////
//Edit card data in collections
const editCard = (req, res) => {
  let { name, team, year } = req.body;
  console.log("*****reqbody", req.body);
  let { id } = req.params;
  // console.log("***HERECADR_ID******", id);
  req.app
    .get("db")
    .cards.edit_card([name, team, year, id])
    .then(cards => res.status(200).send(cards))
    .catch(err => {
      console.log(err);
      res.sendStatus(500);
