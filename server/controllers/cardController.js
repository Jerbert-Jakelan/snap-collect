...
const createCard = (req, res) => {
  ...
  form.parse(req, async (error, fields, files) => {
    ...
    if(possibleMatch && possibleMatch.matches.length > 50) {
      ...
      res.send({message: 'You already have this card!', image: possibleMatch.card.image});
    } else {
      fs.unlink('./server/temp.jpg', err => console.log(err));
      fs.unlink('./server/template.jpg', err => console.log(err));
      let cards = await req.app.get('db').cards.create_card([name[0], team[0], year[0], data.Location, collection[0]]);
